...
function romanToInt(s: string): number {
    ...
    for (let i = 0; i < s.length; i++) {
        ...
        if(s[i] === 'I' && twoLetterRoman.I.includes(s[i+1]) || s[i] === 'X' && twoLetterRoman.X.includes(s[i+1]) || s[i] === 'C' && twoLetterRoman.C.includes(s[i+1])) {
            let r = s[i] + s[i+1];
            result = result + roman.get(r)!;
            i++;
        } else {
...
const symbolicValue: { [key: string] : number } = {
    I: 1,
    V: 5,
    X: 10,
    L: 50,
  ...
  function romanToInt2(s: string): number {
    ...
    while (index < stringList.length) {
        // If current value is less than next value, then it must be merged together
      if (
